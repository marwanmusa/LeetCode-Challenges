...
class Solution:
    ...
    def findDiagonalOrder(self, matrix: List[List[int]]) -> List[int]:
        ...
        # Initialize variables for the matrix dimensions
        m = len(matrix)
        n = len(matrix[0])

        # Initialize variables for the direction of traversal and the current position
        ...
        while len(result) < m * n:
            ...
            # Update the current position based on the direction of traversal
            if direction == 1:
                if j == n - 1:
                    i += 1
                    direction = -1
